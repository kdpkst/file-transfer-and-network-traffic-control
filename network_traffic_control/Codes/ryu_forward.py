...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    @set_ev_cls(ofp_event.EventOFPSwitchFeatures,
                CONFIG_DISPATCHER)  # the decorator tells Ryu when the decorated function should be called
    ...
    def _packet_in_handler(self, ev):
        ...
        eth = pkt.get_protocols(ethernet.ethernet)[0]  # return a list of protocols that matches to 'ethernet.ethernet'
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if msg.buffer_id != ofproto.OFP_NO_BUFFER:  # buffer ID of the packet buffered on OpenFlow. if not buffered, ofproto.OFP_NO_BUFFER is specified
        ...
        data = None
        if msg.buffer_id == ofproto.OFP_NO_BUFFER:  # data is only used when OFP_NO_BUFFER is specified for buffer_id. when switch buffer is used, this is omitted.
            data = msg.data
